...
Private Sub ReadButton1_Click()
   ...
   Else
      ...
      If Nocell Is Nothing Then
         MsgBox "見つかりませんでした。会員番号に誤りがないか確認してください。誤りがない場合は名簿に登録がまだ完了していないので、新規のチェックボックスにチェックを入れて会員番号と名前を手入力してください。"
      
      Else
         NameTextBox = Nocell.Offset(0, 1).Value
         TextBox1 = Nocell.Offset(0, 3).Value
         
      End If
         
      End With
      
         If TextBox1 = "未確認" Then
            MsgBox "スポーツ保険の加入が未確認です。会計の前に保険への加入を案内してください。"
